...
<%@ page import="com.aurionpro.bankmvc.entity.TransactionType" %>
<%
...
<main class="d-flex flex-nowrap h-100">
    ...
    <div class="d-flex flex-column flex-shrink-0 p-3 text-bg-dark" style="width: 280px;">
        ...
        <ul class="nav nav-pills flex-column mb-auto">
            ...
            <li>
                <a href="TransferForm" class="nav-link text-white active ">
    ...
    <%--    allert --%>
    <c:set value="${requestScope.get('moneyTransferred')}" var="moneyTransferred" scope="request"/>
    <c:if test="${not empty moneyTransferred}">
        <script>
            window.addEventListener("load",function(){
                alert("${moneyTransferred}");
            });
        </script>
    </c:if>
    ...
    <c:set scope="request" var="disableTransferType" value="${requestScope.get('disableTransferType')==null?false:requestScope.get('disableTransferType')}"/>
    ...
    <div class="container ">
            <div class="py-5 text-center">
                ...
                <p class="lead">New Transfer Funds.</p>
            </div>

            <div class="container-fluid d-flex justify-content-center">
                <div class="col-md-7 col-lg-8">
                    <h4 class="mb-3">Transfer funds form</h4>

                    <form class="needs-validation" novalidate action="ValidateTransferRequest" method="post">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label for="transferTypeLable" class="form-label" >Account Type</label>
                                <select name="transferType" class="form-select" id="transferTypeLable" required
                                <c:if test="${disableTransferType}">
                                    disabled
                                </c:if>>
                                    <option value="${TransactionType.credit.name()}">${TransactionType.credit.name()}</option>
                                    <option value="${TransactionType.debit.name()}">${TransactionType.debit.name()}</option>
                                    <option value="${TransactionType.transfer.name()}">${TransactionType.transfer.name()}</option>
                                </select>
                            ...
                            <div class="col-md-8">
                                <label for="receiverAccountNumber" class="form-label">Account Number</label>
                                <div class="input-group has-validation">
                                    <span class="input-group-text">@</span>
                                    <input  value="${requestScope.get("receiverAccountNumber")==null?"":requestScope.get("receiverAccountNumber")}" name="receiverAccountNumber" type="number" class="form-control" id="receiverAccountNumber" placeholder="Account number only if transfer" required
                                    <c:if test="${disableTransferType}">
                                           disabled
                                    </c:if>>
                                </div>
...
<%--                            <div class="col-md-4 d-flex align-items-end">--%>
                                <button class="w-100 btn btn-primary btn-lg" type="submit"  <c:if test="${disableTransferType}">
                                    disabled
                                </c:if>>Check customer ID</button>
<%--                            </div>--%>
                        </div>
                    </form>

                    <form class="needs-validation" novalidate action="MakeTransfer" method="post">
                        <div class="row g-3">
                            <input type="text" name="transferType" value="${requestScope.get("transferType")==null?"":requestScope.get("transferType").name()}" hidden>
                            <input type="number" name="receiverAccountNumber" value="${requestScope.get("receiverAccountNumber")==null?"":requestScope.get("receiverAccountNumber")}" hidden>

                            <div class="col-12">
                                <label for="amount" class="form-label">Amount</label>
                                <input name="amount" type="number" class="form-control" id="amount" placeholder="0"  required <c:if test="${disableForm}">
                                       disabled
                                </c:if>>
                                <div class="invalid-feedback">
                        ...
                        <hr class="my-4">
                        <button class="w-100 btn btn-primary btn-lg" type="submit"  <c:if test="${disableForm}">
                            disabled
                        </c:if>>${requestScope.get("transferType")==null?"select Transfer Type":requestScope.get("transferType")}</button>
                    </form>
                </div>
            </div>
