...
<main class="d-flex flex-nowrap h-100">
    ...
    <div class="d-flex flex-column flex-shrink-0 p-3 text-bg-dark" style="width: 280px;">
        ...
        <ul class="nav nav-pills flex-column mb-auto">
            ...
            <li class="nav-item">
                <a href="PassbookView" class="nav-link text-white active">
    ...
    <div class="container ">
            <div class="py-5 text-center">
                ...
                <p class="lead">
                    Account Number:<c:out value="${sessionScope.get('selectedBankAccount').getAccountNumber()}"/><br>
                    Customer ID:<c:out value="${sessionScope.get('selectedBankAccount').getCustomerId()}"/><br>
                    Account Type:<c:out value="${sessionScope.get('selectedBankAccount').getBankAccountType().name()}"/><br>
                    Balance:<c:out value="${sessionScope.get('selectedBankAccount').getBalance()}"/><br>
                    <c:if test="${sessionScope.get('selectedBankAccount').getBankAccountType().equals(BankAccountType.current)}">
                        Limit:<c:out value="${sessionScope.get('selectedBankAccount').getMin_overdue_limit()}"/>
                    </c:if>
                    <c:if test="${sessionScope.get('selectedBankAccount').getBankAccountType().equals(BankAccountType.saving)}">
                        Minimum Account:<c:out value="${sessionScope.get('selectedBankAccount').getMin_overdue_limit()}"/>
                    </c:if>
                </p>
            </div>
            </div>
        ...
        <div class="container-fluid d-flex justify-content-center">
            <table id="customer_table" class="display nowrap" style="width:100%">
                ...
                <tbody>
                <c:set var="transactions" scope="session" value="${requestScope.get('transactions')}"/>
                <c:forEach items="${transactions}" var="transaction">
                    <tr>
                        <td><c:out value="${transaction.getTransactionId()}"/></td>
                        <td><c:out value="${transaction.getSenderAccountNumber()}"/></td>
                        <td><c:out value="${transaction.getReceiverAccountNumber()}"/></td>
                        <td><c:out value="${transaction.getTransactionType().name()}"/></td>
                        <td><c:out value="${transaction.getAmount()}"/></td>
                        <td><c:out value="${transaction.getCurrent_balance()}"/></td>
                        <td><c:out value="${transaction.getNew_balance()}"/></td>
